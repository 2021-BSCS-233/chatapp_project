...
class Messages extends StatelessWidget {
  ...
  Messages({required this.toggleMenu, required this.userData});
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Widget>(
      ...
      builder: (context, snapshot) {
        ...
        // return Container(child: Center(child: Material(child: Text('Refreshing')),),);
      },
    );
  }

  Future<Widget> _buildContent(BuildContext context) async {
    await getChats(userData['_id']);
    await getFriends(userData['_id']);
      ...
      body: Container(
        ...
        child: Column(
          children: [
                  ...
                  itemBuilder: (context, index) {
                    return Container(
                      ...
                      child: Center(
                        child: InkWell(
                          onTap: () {
                            print('profole: ${friends[index]['friend_id']}');
                          },
                          child: Stack(
                            children: [
                              Container(
                              ...
                              Positioned(
                                ...
                                child: StatusIcon(
                                  icon_type:
                                  friends[index]['display_status'] == '' && friends[index]['status'] != 'Online'
            ...
            Expanded(
              child: Obx(() => ListView.builder(
                    itemCount: CLenght.value,
                    shrinkWrap: true,
                    itemBuilder: (context, index) {
                      return chats[index]['chat_type'] == 'dm'
                          ? UserChatTile(
                              user_data: chats[index]['users'][0],
                              top_message: chats[index]['latest_message'],
                              chat_page_id: chats[index]['chat_id'],
                              chat_type: chats[index]['chat_type'],
                              log_press_menu: toggleMenu)
                          : Container(
                              height: 50,
                              width: 100,
                              child: Center(
